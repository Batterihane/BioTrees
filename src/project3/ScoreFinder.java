...
public class ScoreFinder {
    ...
    public int findScore(String input, String fold){
        int score = 0;
        int iud=0, ilr = 0;
        ...
        for(int i=0; i<fold.length();i++){
            ...
            if (input.charAt(i)=='h'){ //if char at i is an h, try to score it
                int jud=iud, jlr=ilr;
                ...
                for(int j = i ;j<fold.length();j++){
                    ...
                    if(((iud-jud==0||ilr-jlr==0)&&(Math.abs(iud-jud)==1||Math.abs(ilr-jlr)==1)) //if one of ud and lr is 0 and one of them is 1 in absolute terms
                            &&input.charAt(j)=='h' //and the char at j is an h
